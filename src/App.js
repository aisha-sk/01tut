...
function App() {
  ...
  useEffect(() => {
    ...
    setTimeout(() => fetchItems(), 2000); // to simulate time being taken to fetch
  ...
  const handleSubmit = (e) => {
    ...
    addItem(newItem);
    setNewItem('');
    /*(e) is the parameter of the function, which represents the event object generated by the form submission.

    e.preventDefault();: This line prevents the default behavior of the form submission. By default, when a form is submitted, the page is refreshed or redirected, which is not desired in this case. The e.preventDefault() prevents that default behavior, allowing us to handle the form submission programmatically. */
  }
